...
const ProfileContainer = ({
  ...
  const renderFollowingContainer = () => {
    return (
      <FollowingContainer
        followings={slicedFollowings()}
        nextFollowings={nextFollowings}
          ...
          <Row className='mt-5 align-items-center justify-content-center'>
            <Col>
              <ProfilePicCard
                ...
                profilePic={photoUrl}
                favConsole={user.fav_console}
                favGenre={user.fav_genre}
                favGame={user.fav_game}
              />

              {authUser ? (
                authUser.id === user.id ? (
                  <Link to={`/users/${authUser.id}/edit`}>
                    <button type='button' className='btn-nes primary'>
                      Edit User Info
                    </button>
                  </Link>
                ) : (
                  <FollowButton
                    ...
                    addFollowerToProfile={addFollowerToProfile}
                    removeFollowerFromProfile={removeFollowerFromProfile}
                  />
                )
              ) : (
                <>
            ...
            <Col>
              ...
              <Row>
                <Col className='d-flex flex-wrap justify-content-around slide-in'>
                  <FavoritesContainer
                    slicedFavorites={slicedGames()}
                    gameUserId={user.id}
                    removeFavoriteFromProfile={removeFavoriteFromProfile}
                  />
                </Col>
              </Row>
            </Col>
